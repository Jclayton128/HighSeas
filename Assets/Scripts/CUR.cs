...
public static class CUR : object
{
    ...
    static public GameObject FindNearestGameObjectWithTag(Transform posToSearchFrom, string tagName, float maxSearchDistance, GameObject gameObjectRootToIgnore)
    {
        GameObject closestTarget = null;
        GameObject[] possibleTargets;
        possibleTargets = GameObject.FindGameObjectsWithTag(tagName);
        ...
        foreach (GameObject currentTargetBeingEvaluated in possibleTargets)
        {
            if (currentTargetBeingEvaluated.transform.root.gameObject == gameObjectRootToIgnore) continue;
            Vector3 diff = currentTargetBeingEvaluated.transform.position - position;
            float curDistance = diff.magnitude;
            if (curDistance < distance)
            {
